 ...
 */
app.controller('groupUserRegCtrl', function ($scope, $state, $stateParams,$http,$httpParamSerializerJQLike,$window,$rootScope,$interval,$ionicPopup,$ionicLoading) {
    ...
    $http({
        ...
        //本次比赛需要的证件
        credentialReuire = $scope.userInfo.credentialReuire
        if($scope.userInfo.credentialReuire != null && $scope.userInfo.credentialReuire != "") {
            $scope.needPhotoNumber = JSON.parse($scope.userInfo.credentialReuire).needPhotoNumber

            $scope.credentialReuire = JSON.parse($scope.userInfo.credentialReuire)
    ...
    $scope.toRegNext = function (id,number,mobile,level1,level2) {
        submitInfo.teamName = $scope.userInf.name
        submitInfo.uname = $scope.userInfo.realname
        submitInfo.gender = gender
        submitInfo.competitionType = 1
        ...
        submitInfo.verifyCode = $scope.userInf.indCode
        ...
        $http({
            ...
            function (res) {
                ...
                if (res.result == 0) {
                    if($scope.order == null){
                        $rootScope.teamName = $scope.userInf.name
                        $rootScope.coachName = $scope.userInf.coach
                        $state.go('selectedProject', {competitionId: $scope.competitionId})
                    }
                    if($scope.order != null){
                        $state.go('pay', {competitionId: $scope.competitionId})
                    }
