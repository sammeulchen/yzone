 ...
 */
app.controller('doubleRankCtrl', function ($scope,$state,$http,$ionicPopup,$rootScope,$window) {
    ...
    $http.get(SITE_SUFFIX+'api/ucenter/level/get').success(function (response) {
        ...
        console.log($scope.doubleLevel)
        for(var i in $scope.rankingGrades){
            $scope.selectRank.rank=$scope.rankingGrades[$scope.doubleLevel.parentLevel-1];
        }
        rank = $scope.doubleLevel.parentLevel
    ...
    $http({
        ...
        method:'get',
        params:{level:rank,gender:gender,category:2}
        ...
        if(response.result== 0){
            ...
            $scope.loadMore = function () {
                ...
                $http({
                    url:SITE_SUFFIX+'api/ucenter/level/rank',
                    method:'get',
                    params:{level:rank,gender:gender,category:2,page:$scope.page}
                }).success(function (response) {
                    console.log(response)
                    if(response.result== 0){
    ...
    $scope.setBg2 = function (le) {
        var b = '';
        if(le < rank2){b = '#D1E5F7';}
        if(le >= rank2){b = '#28A7E1'}
        return {background:b}
    }
    $scope.setBg3 = function (le) {
        var c = '';
        if(le <rank2){c = '#D1E5F7'}
        if(le >= rank2 && le<rank4){c = '#28A7E1';}
        if(le == rank4){ c = '#E2EEC5'}
        if(le > rank4){ c = '#009F40'}
        return {background:c}
    }
    $scope.setBg4 = function (le) {
        var d = '';
        if(le<rank3){d = '#D1E5F7';}
        if(le >= rank3 && le<rank4){ d = '#28A7E1'}
        if(le >= rank4 && le < rank5){ d = '#E2EEC5'}
        if(le >= rank5){ d = '#009F40'}
        return {background:d}
    }
    $scope.setBg5 = function (le) {
        var e = '';
        if(le<rank3){e = '#D1E5F7';}
        if(le >= rank3 && le<rank4){ e = '#28A7E1'}
        if(le >= rank4 && le < rank5){ e = '#E2EEC5'}
        if(le >= rank5 && le < rank7){ e = '#009F40'}
        if(le == rank7){ e = '#F8C6AD'}
        if(le > rank7){ e = '#E84518'}
        return {background:e}
    }
    $scope.setBg6 = function (le) {
        var f = '';
        if(le<rank4){f = '#D1E5F7';}
        if(le >= rank4 && le < rank6){ f = '#E2EEC5'}
        if(le >= rank6 && le < rank7){ f = '#009F40'}
        if(le == rank7){ f = '#F8C6AD'}
        if(le > rank7){ f = '#E84518'}
        return {background:f}
    }
    $scope.setBg7 = function (le) {
        var g = '';
        if(le < rank6){g = '#E2EEC5';}
        if(le >= rank6 && le<rank7){ g = '#009F40'}
        if(le >= rank7 && le< rank8){ g = '#F8C6AD'}
        if(le >= rank8 && le<= rank10){ g = '#E84518'}
        if(le ==rank10){ g = '#D4CEC5'}
        if(le >rank10){ g = '#7B776E'}
        return {background:g}
    }
    $scope.setBg8 = function (le) {
        h = '';
        if(le<rank7){h = '#E2EEC5';}
        if(le >= rank7 && le < rank9){ h = '#F8C6AD'}
        if(le >= rank9 && le< rank10){ h = '#E84518'}
        if(le >=rank10 && le< rank11){ h = '#D4CEC5'}
        if(le >= rank11){ h = '#7B776E'}
        return {background:h}
    }
    $scope.setBg9 = function (le) {
        var i = '';
        if(le<rank8){i = '#F8C6AD';}
        if(le >= rank8 && le < rank10){ i = '#E84518'}
        if(le >=rank10 && le< rank11){ i = '#D4CEC5'}
        if(le>rank11 ){ i = '#7B776E'}
        return {background:i}
    }
    $scope.setBg10 = function (le) {
        var j = '';
        if(le<rank10){j = '#F8C6AD';}
        if(le >= rank10 && le < rank11){ j = '#D4CEC5'}
        if(le > rank11){ j = '#7B776E'}
        return {background:j}
    }
    $scope.setBg11 = function (le) {
        var k = '';
        if(le<rank11){k = '#D4CEC5';}
        if(le >=rank11){ k = '#7B776E'}
        return {background:k}
    }
    $scope.setBg12 = function (le) {
        var l = '';
        if(le<rank12){l = '#D4CEC5'}
        if(le>=rank12){l = '#7B776E'}
        return {background:l}
    }
    ...
    $scope.toDoubleRanking = function () {
        $window.sessionStorage.levelId= 2
